...
<body>
  ...
  //?>
    ...
    <h3> Input Obat Keluar </h3>

    <form method="POST" action="prosesobatkeluar.php">
        <!--Kode Transaksi-->
        <div class="mb-3">
        ...
        <!--ID Instansi-->
        <div class="mb-3">
            <label for="id_instansi" class="tambahdata form-label">ID Instansi </label>
            <select class="form-select" name="id_instansi" aria-label="Default select example">
                <option class="pt-2" value="">Pilih</option>
                <?php 
              $id_instansi = mysqli_query($conn, "SELECT id_instansi FROM obatkeluar order by id_instansi ASC;");
              while($data = mysqli_fetch_array($id_instansi)){
            ?>
                <option value="<?php echo $data['id_instansi'] ?>">
